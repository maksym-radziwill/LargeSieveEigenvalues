...
#ifdef USEFLOAT
void ssyev_(char *jobz, char *uplo, int *n, TYPE *a, int *lda, 
           TYPE *w, TYPE *work, int *lwork, int *info);

void sdisna_(char *job, int *m, int *n, TYPE * d, TYPE *sep, int *info); 

TYPE slamch_(char *c); 
#else
void dsyev_(char *jobz, char *uplo, int *n, TYPE *a, int *lda, 
           TYPE *w, TYPE *work, int *lwork, int *info);

void ddisna_(char *job, int *m, int *n, TYPE * d, TYPE *sep, int *info); 
...
void usage (char * s){
  printf("Usage: %s q [-n n] [-v] [-r] [-m] [-M mem]\n"
	 ...
	 "                               -M set a cap on memory usage in Megabytes\n", s);
  exit(0); 
}



TYPE machine_precision(){
  char jobz = 'E';
#ifdef USEFLOAT
  return slamch_(&jobz);
#else
...
TYPE * eigenvectors_precision(TYPE * w, int n){ 
  char * job = "Eigenvectors";
  int  N = n;
  TYPE * rcondz = calloc(N, sizeof(TYPE));
  TYPE * zerobd = calloc(N, sizeof(TYPE));
  TYPE anorm = max (abs(w[0]), abs(w[n-1])); 
  int info;
  TYPE * W = w; 

#ifdef USEFLOAT
  sdisna_(job, &N, &N, W, rcondz, &info);
#else
...
int log_TYPE(TYPE w){
  int i = 1;
  while(power_TYPE(10,i)*w < 1)
    i += 1; 
  return i; 
}

TYPE eigenvalues_precision(TYPE * w, int n){
  TYPE anorm  = max (abs(w[0]), abs(w[n-1]));
  return machine_precision()*anorm; 
}

void print_eigenvectors(TYPE * matrix, TYPE * w, int n){

  TYPE * zerobd = eigenvectors_precision(w,n); 

  int PREC = log_TYPE(eigenvalues_precision(w,n)) - 1; 
  int PREC2 = log_TYPE(1 / (TYPE) n) - 1; 

  for(int i = 0; i < n; i++){
    printf("Eigenvector with eigenvalue %.*f (normalized %.*f)\n", PREC, w[i],
	   PREC + PREC2, w[i] / (TYPE) n);
...
void print_eigenvalues_precision(TYPE * w, int n){
  int PREC = log_TYPE(eigenvalues_precision(w,n));
  int PREC2 = log_TYPE(1/(TYPE) n); 
...
void print_eigenvalues(TYPE * w, int n, int q){
  ...
  for(int i = 0; i < n; i++){
    if(w[i] < 0) w[i] = 0; 
    printf("%6d: %.*f (normalized %.*f) \n", i + 1, PREC, w[i], PREC + PREC2, w[i] / (TYPE) n ); 
  }

  line_break(); 
}

void print_plain_eigenvalues(TYPE * w, int n){
  int PREC = log_TYPE(eigenvalues_precision(w,n)) - 1; 
  int PREC2 = log_TYPE(1 / (TYPE) n) - 1; 
...
int main(int argc, char ** argv)
{
  ...
  while ((c = getopt (argc, argv, "n:vrmeM:")) != -1)
    switch (c){
...
#ifdef USEFLOAT
  ssyev_(&jobz, &uplo, &n, a, &lda, w, &wwork, &lwork, &info);
#else
